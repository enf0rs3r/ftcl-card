...
# Обработчик кнопки "🎁 Паки" (добавляем выбор)
...
async def open_normal_pack(message: types.Message):
    await open_pack(message)  # Открываем обычный пак

# Обработчик бесконечного пака
@router.message(lambda message: message.text == "♾ Бесконечный пак")
async def open_endless_pack(message: types.Message):
    await endless_pack(message)  # Открываем бесконечный пак
